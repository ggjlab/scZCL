...
#' Zebrafish Cell Landscape mapping
#' description
...
scZCL <- function(scdata,numbers_plot=3){
  ...
  result[['scZCL']]<-scblast.result
  result[['scZCL_probility']]<-cors_out
  return(result)
}
...
#' @export
plotZCL <- function(zcl_result,interactive_plot=F, numbers_plot=3, col_font_size = 1, row_font_size=8, show_col=T,show_bar=T, show_tree = T){
  data(ref.expr)
  cors <- zcl_result$cors_matrix
  cors_index <- apply(cors,2,gettissue,numbers_plot)
  cors_index <- sort(unique(as.integer(cors_index)))
  ...
  colnames(cors_out)<- c("Cell","Cell type","Score")
  cors_out <- as.data.frame(cors_out %>% group_by(Cell) %>% top_n(n=numbers_plot,wt=Score))
  zcl_result$scZCL_probility <- cors_out
  zcl_result$top_cors <- numbers_plot
